...
process.argv.slice(2).forEach(arg => {
    ...
    let running = 0;
    let stuff = [];
    for (var s = 0; s < config[arg].schemas.length; s++) {
        let sql = fs.readFileSync(config[arg].schemas[s], 'utf8');
        // Lose all comments.
        sql = sql.replace(/--.*?\n/g, '')
                 .replace(/\/\*(.|\n)*?\*\//, '');
    ...
    function finish() {
        // Perform cleanup (drop everything that wasn't defined)
        connection.query('SHOW TABLES', (err, rows) => {
            running = rows.length;
            rows.map(row => {
                for (let tbl in row) {
                    tbl = row[tbl];
                    if (stuff.indexOf(tbl) == -1) {
                        connection.query('DROP TABLE ' + tbl, err => {
                            if (err) {
                                connection.query('DROP VIEW ' + tbl, () => {
                                    check();
                                });
                            } else {
                                check();
